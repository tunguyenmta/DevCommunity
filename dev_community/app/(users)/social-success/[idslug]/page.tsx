...
const Page = ({ params }: { params: { idslug: string } }) => {
  ...
  const fetchUserToken = async () => {
    ...
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_BASE_URL}api/v1/authenticate/social-success/${params.idslug}`
    );
    if (!res.ok) {
    ...
    const result1 = await fetch("htpp://localhost:3000/pages/api/auth", {
      ...
      body: JSON.stringify({ token: data.idToken, refreshToken: data.idRefreshToken, maxAge: data.refreshTokenValidSecond }),
    });
    ...
    if (decodedToken) {
      sessionStorage.removeItem("isThirdParty");
  ...
  if (isLoading) {
    return <div>Unauthorized</div>; // Show loading or null while checking
  }
  return (
    <div>
      ...
      <div className="py-3">
        <h2 className="text-center font-semibold text-[#0094ff]">
          Login Successfully
        </h2>
      </div>
    </div>
