...
import walletAdd from "../../../../public/asstets/icons/Themes (13)/vuesax/linear/additem.svg";
...
import ExampleCreator from "./exampleEdit";
...
const ListExample: React.FC<ListExampleProps> = ({
  ...
  const [listExampleTemp, setListExampleTemp] = useState<ExampleTempProps[]>(
    list.map((item, ind) => {
      return {
        example: item, // Corrected object structure according to ExampleTempProps
        tempId: ind + 1, // Ensures unique temporary IDs for handling UI operations
      };
    })
  );
  ...
  useEffect(() => {
    if (initialList) setListExample(initialList);
  }, [initialList]);
  ...
  const handleDuplicateExample = (tempId: number) => {
    ...
    if (exampleToDuplicate) {
      ...
      // Clone the example with a new tempId but exclude the id property from being copied
      let duplicatedExample: ExampleTempProps = {
        ...exampleToDuplicate,
        tempId: newTempId,
        example: {
          ...exampleToDuplicate.example,
          id: undefined, // Explicitly setting id to undefined
        },
      };
  ...
  const handleArchiveExample = (tempId: number) => {
    // Find the example to archive
    const exampleToArchive = listExampleTemp.find(
      (example) => example.tempId === tempId
    );
    if (exampleToArchive) {
      // Assuming you have a way to archive the example
      // You can remove the example from the list or update its status
      // Ensure the UI reflects the change
  ...
  // Utility function to generate the next tempId (this is just a placeholder, implement according to your logic)
  const getNextTempId = () => {
    return (
      Math.max(
        0,
    ...
    if (!example.tempId) {
      ...
      const updatedList = listExampleTemp.map((item) => {
        if (item.tempId === example.tempId) {
          // Spread operator to copy properties except tempId
  ...
  return (
    <>
      ...
      <div
        id="example-creator"
        className="bg-white rounded-[12px] relative mb-[200px]"
      >
        ...
        {listExampleTemp.length == 0 ? (
          ...
          <div className="flex flex-wrap gap-10 p-5 min-h-screen">
                      ...
                      >
                        ...
                        <div>
                          ...
                          <input
                            ...
                            placeholder="archive"
                            type="text"
                            className="border-[1px] border-[#DCDCDC] cursor-pointer rounded-[8px] w-full py-2 px-4 mt-2"
                          />
                        </div>
                        <div className="flex justify-end gap-5 pt-4">
                          <button onClick={()=>{
                            setActiveMenuId(null);
                            setIsModalOpen(false)}}
                             className="bg-[#F4F4F4] rounded-[8px] font-semibold px-4 border-[1px] py-2">
                            Cancel
                          </button>
                          <button
                            onClick={() => {
